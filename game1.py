...
cursor.execute("INSERT INTO game_session (user_id,game_id,outcome) VALUES (?,?,?)", (user_id,1,'win'))
conn.commit()
...
def enemy_spawner():
    while True:
        ...
        enemy = Enemy(randomX, randomY, 75, 75, "assets/enemy-Sheet.png", 1.5)
        player_center = player.get_center()
...
while True:
    ...
    if player.health <= 0:
       if not is_game_over:
           is_game_over = True
       cursor.execute("INSERT INTO score (user_id, game_id, score) VALUES (?, ?, ?)", (user_id, 1, score))
       conn.commit()

        # Update end time in game_session
       ...
       cursor.execute("SELECT score FROM leaderboard WHERE user_id = ? AND game_id = ?", (user_id, 1))
       result = cursor.fetchone()

       if result is None:
          # No existing record, insert new one
          cursor.execute("INSERT INTO leaderboard (user_id, game_id, score) VALUES (?, ?, ?)", (user_id, 1, score))
       elif score > result[0]:
          # Score is higher, update record
          cursor.execute("UPDATE leaderboard SET score = ? WHERE user_id = ? AND game_id = ?", (score, user_id, 1))
